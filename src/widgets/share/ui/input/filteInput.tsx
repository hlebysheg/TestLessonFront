...
export const FileInput = ({ onChange, labelText, error}: InputProps) => {
    ...
    const ResetButton = () => <StyledLabel onClick={handleClickReset}><CrossRed /></StyledLabel>;
    return (
        <InputWrapper>
            <StyledLabel>{error ?? labelText}</StyledLabel>
            <Rectangle onClick={() => handleClick} onDragOver={onDragOver} onDrop={handleDrop} style={{borderColor: !!error?'red':'#E4E5E7'}}>
                <StyledLabel style={{paddingLeft: '20px'}}>{title} {isFile&&<ResetButton/>}</StyledLabel>
                <StyledButton onClick={handleClick} type="button">
                    <UploadIcons/>
                </StyledButton>
            </Rectangle>
            <StyledInputBar ref={actualBtnRef} id={labelText} hidden type="file" onChange={handleChange} />
        </InputWrapper>
    )
