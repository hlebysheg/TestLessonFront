...
export const $isPayFormOpen = createStore(false)
  .on(validateStandartFormFx.doneData, () => true)
  .reset(submitted);
...
export const $errorBik = restore<Error>(validateFxBik.failData, null).reset(
  changedBik
);
...
export const $errorBankName = restore<Error>(
  validateFxBankName.failData,
  null
).reset(changedBankName);
...
export const $errorCheckingAccount = restore<Error>(
  validateFxCheckingAccount.failData,
  null
).reset(changedCheckingAccount);

//correspondentAccount
export const changedCorrespondentAccount = createEvent<string>();
export const validateFxCorrespondentAccount = createEffect(
  (form: PayFormType) => {
    if (form.correspondentAccount === "" || form.correspondentAccount == null) {
      throw new Error("Введите значение");
    }
    if (!isNumeric(form.correspondentAccount))
      throw new Error("Введите цифры!!!!");
    return true;
  }
);
export const $errorCorrespondentAccount = restore<Error>(
  validateFxCorrespondentAccount.failData,
  null
).reset(changedCorrespondentAccount);
...
export const $payForm = combine({
  ...
  correspondentAccount: restore(changedCorrespondentAccount, "").reset(
    submitted
  ),
}).reset(submitted);
