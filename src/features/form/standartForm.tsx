import { allSettled, combine, createEffect, createEvent, createStore, fork, forward, restore, sample, scopeBind } from "effector";
...
export const $errorInn = restore<Error>(validateFxInn.failData, null).reset(changedInn)
...
export const $errorInnFile = restore<Error>(validateFileInn.failData, null).reset(changedInnFile)
...
export const $errorOgrip = restore<Error>(validateFxOgrip.failData, null).reset(changedOgrip, submitted)
...
export const $errorOgripFile = restore<Error>(validateFileOgrip.failData, null).reset(changedOgripFile)
...
export const $errorRegisterDate = restore<Error>(validateFxDateReqire.failData, null).reset(changedRegisterDate)
...
export const $errorEgripFile = restore<Error>(validateFileEgrip.failData, null).reset(changedEgripFile)
...
export const $errorRentFile = restore<Error>(validateFileRent.failData, null).reset(changedRentFile)
...
validateStandartFormFx.doneData.watch(() => console.log($standartForm.getState()))
