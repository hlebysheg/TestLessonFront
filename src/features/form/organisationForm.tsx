import { createEvent, createEffect, restore, sample, combine, createStore } from "effector";
...
export const validateFxOranisationFullName = createEffect((form: OrganisationFormType) => {
...
export const $errorFull = restore<Error>(validateFxOranisationFullName.failData, null).reset(changedOrganisationFullName)
  
//Short
export const changedOrganisationShortName = createEvent<string>()
export const validateFxOranisationShortName = createEffect((form: OrganisationFormType) => {
    if(form.organisationShortName === "" || form.organisationShortName == null){
        throw new Error('Введите значение')
    }
    return true;
})
  
export const $errorShort = restore<Error>(validateFxOranisationShortName.failData, null).reset(changedOrganisationShortName)

//data
//дата регистрации
export const changedRegisterDateOrganisation = createEvent<string>()
export const validateFxDateOrganisationReqire = createEffect((form: OrganisationFormType) => {
    if(form.date === "" || form.date == null){
        throw new Error('Введите значение')
    }
    return true;
})
export const $errorRegisterDateOrganisation = restore<Error>(validateFxDateOrganisationReqire.failData, null).reset(changedRegisterDateOrganisation)

export const $organisationForm = combine({
    organisationFullName: restore(changedOrganisationFullName, '').reset(submitted),
    organisationShortName: restore(changedOrganisationShortName, '').reset(submitted),
    date: restore(changedRegisterDateOrganisation, '').reset(submitted),
}).reset(changeFormModeEvent).reset(submitted)//сброс при смене мода

//ИНН: 3664069397, ОГРН: 1053600591197,
export const validateOrganisationFormFx = createEffect<OrganisationFormType, boolean>()
