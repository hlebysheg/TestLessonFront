...
fetchFormFx.use(async (store: fetchdata) => {
  ...
  org.append(
    "Organisation.OrganisationFullName",
    store.$organisationForm.organisationFullName
  );
  org.append(
    "Organisation.OrganisationShortName",
    store.$organisationForm.organisationShortName
  );
  ...
  org.append(
    "PayInfo.CorrespondentAccount",
    store.$payForm.correspondentAccount
  );
  ...
  if (req.status != 200) throw new Error("send error");
  return true;
});
